...
const PerfilOutraPessoa = ({ navigation, route }) => {
  ...
  const isFollowing = (followers, id) => {
    return followers.some(follower => follower.id === id);
  }
  ...
  const follow = async () => {
    try {
      ...
      if (response.status == 200) {
        ...
        setUser(data);
        setFollowing(isFollowing(data.followers, storedUser.id));
      }
    } catch (error) {
      Alert.alert("Erro ao seguir o usuário.");
    }
  }
  ...
  const unfollow = async () => {
    try {
      ...
      Alert.alert("Erro ao deixar de seguir o usuário.");
    }
  }
  ...
  return (
    <>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Image
          source={{ uri: user.backgroundPictureURL }}
        ...
        {
          following ?
            <Pressable style={styles.followButton} onPress={unfollow}>
              <Text style={styles.editarPerfilTexto}>Deixar de Seguir</Text>
            </Pressable>
            :
            <Pressable style={styles.followButton} onPress={follow}>
              <Text style={styles.editarPerfilTexto}>Seguir</Text>
            </Pressable>
        }
        <View style={styles.infoContainer}>
          ...
          <View style={styles.seguindoContainer}>
            <Text style={styles.seguindoNumeros}><Text style={styles.numero}>{user.followingCount}</Text> Seguindo</Text>
            <Text style={styles.seguindoNumeros}><Text style={styles.numero}>{user.followersCount}</Text> Seguidores</Text>
          </View>
        </View>
